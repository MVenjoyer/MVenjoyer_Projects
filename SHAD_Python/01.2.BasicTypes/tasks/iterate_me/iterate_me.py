...
def get_max_element_index(elements: list[int]) -> int | None:
    ...
    for index, item in enumerate(elements):
        if item >= elements[max_value_idx]:
            max_value_idx = index
    return None if elements == [] else max_value_idx
...
def get_every_second_element(elements: list[int]) -> list[int]:
    ...
    """
    return [item for index, item in enumerate(elements) if index % 2 != 0]
...
def get_by_index(elements: list[int], i: int, boundary: int) -> int | None:
    ...
    """
    return None if (value := elements[i]) <= boundary else value
