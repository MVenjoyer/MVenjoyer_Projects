...
def get_unique_page_ids(records: list[tp.Mapping[str, tp.Any]]) -> set[int]:
    ...
    """
    return {ev["PageID"] for ev in records}
...
def get_unique_page_ids_visited_after_ts(records: list[tp.Mapping[str, tp.Any]], ts: int) -> set[int]:
    ...
    """
    return {ev["PageID"] for ev in records if ev["EventTime"] > ts}
...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    return {ev["UserID"] for ev in records if ev["EventTime"] > ts and ev["PageID"] == page_id}
...
def get_events_by_device_type(
    ...
    """
    return [ev for ev in records if device_type == ev["DeviceType"]]
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [ev["RegionID"] if ev["RegionID"] is not None else DEFAULT_REGION_ID for ev in records]
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    return [ev["RegionID"] for ev in records if ev["RegionID"] is not None]
...
def get_keys_where_value_is_not_none(r: tp.Mapping[str, tp.Any]) -> list[str]:
    ...
    """
    return [keys for keys, values in r.items() if values is not None]
