...
func TestEventRepository_GetLastEventBySensorID(t *testing.T) {
	...
	t.Run("ok, get Sensor history", func(t *testing.T) {
		er := NewEventRepository()
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		actualEvent, err := er.GetSensorHistory(ctx, sensorID, time.Now().Add(-time.Hour), time.Now())
		assert.NoError(t, err)
		assert.NotNil(t, actualEvent)
		assert.Equal(t, 2, len(actualEvent))
	})

	t.Run("fail, invalid time stamps(history)", func(t *testing.T) {
		er := NewEventRepository()
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		_, err := er.GetSensorHistory(ctx, sensorID, time.Now().Add(+time.Hour), time.Now())
		assert.Error(t, err)
		assert.Equal(t, usecase.ErrInvalidEventTimestamp, err)
	})

	t.Run("fail, event not found(history)", func(t *testing.T) {
		er := NewEventRepository()
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		_, err := er.GetSensorHistory(ctx, 54321, time.Now().Add(-time.Hour), time.Now())
		assert.Error(t, err)
		assert.Equal(t, usecase.ErrEventNotFound, err)
	})
}
